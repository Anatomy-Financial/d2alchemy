...
class Table:
    ...
    def to_d2_table(self) -> "SQLTable":
        ...
        """
        try:
            from py_d2.sql_table import SQLConstraint, SQLTable
        except ImportError:
            raise ImportError("py-d2 is required. Install it with: pip install py-d2")
    ...
    @staticmethod
    def create_d2_diagram(tables: List["Table"]) -> Tuple["D2Diagram", List[Any]]:
        ...
        for table in tables:
            ...
            snake_case = ''.join(['_' + c.lower() if c.isupper() else c for c in table.name]).lstrip('_')
                ...
                if column.constraint.foreign_key:
                    ...
                    try:
                        ...
                        if ref_table_raw in table_name_mapping:
                            ...
                            print(f"Note: Using original table name reference: {ref_table}")
...
if __name__ == "__main__":
    ...
    tables: List[Table] = find_sqlalchemy_models(directory)
    ...
    diagram, connections = Table.create_d2_diagram(tables)

    # Write the diagram to a file
    file_name = "sqlalchemy_schema"
    with open(f"{file_name}.d2", "w") as f:
        f.write(str(diagram))

    # Generate SVG (if d2 is installed)
    try:
        import subprocess

        subprocess.run(
            ["d2", "--layout", "elk", f"{file_name}.d2", f"{file_name}.svg"], check=True
        )
